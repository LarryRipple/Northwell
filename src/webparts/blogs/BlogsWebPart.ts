...
{itemstoshow= +this.properties.items;}
    var items =  sp.web.lists.getByTitle("Site Pages").items.select("Title", "FileRef", "BannerImageUrl", "Description", "OData__TopicHeader").filter(filter).top(itemstoshow).get().then(b => {
console.log(b)
b.forEach(element => {
